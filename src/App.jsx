  ...
  return (
    <div className="app-shell">
      ...
      <main id="top">
        ...
        <section className="collection-strip" id="collections"><div><small>01</small><strong>Prams & Travel Systems</strong><span>Compact to luxury</span></div><div><small>02</small><strong>Car Seats</strong><span>Safety, elevated</span></div><div><small>03</small><strong>Nursery</strong><span>Designed to last</span></div><div><small>04</small><strong>Baby Tech</strong><span>Monitors, sleep & feeding tech</span></div></section>

        <section className="section" id="shop"><div className="section-heading"><div><span className="eyebrow">The premium collection</span><h2>Statement pieces. Serious function.</h2></div><label className="search-box"><span>⌕</span><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search prams, car seats, baby tech..." /></label></div><div className="category-row">{categories.map((item) => <button key={item} className={category === item ? 'active' : ''} onClick={() => setCategory(item)}>{item}</button>)}</div><div className="product-grid premium-grid">{filteredProducts.map((product, index) => <article className="product-card premium-card" key={product.id}><div className={`product-visual tone-${product.tone}`}><div className="visual-index">{String(index + 1).padStart(2, '0')}</div><span className="visual-tag">{product.tag}</span><div className="visual-name">{product.category}</div><img className="product-image" src={product.image} alt={product.name} loading="lazy" /></div><div className="product-info"><div className="product-top"><h3>{product.name}</h3><strong>{money(product.price)}</strong></div><p>{product.description}</p><button onClick={() => addToCart(product)}>Add to bag</button></div></article>)}</div>{filteredProducts.length === 0 && <div className="empty-state">No premium products found. Try another search or category.</div>}</section>

        <section className="editorial-banner"><div><span className="eyebrow">Built around real family life</span><h2>Premium does not mean complicated.</h2></div><p>We focus on the pieces parents use every day: flexible travel systems, safety-led car seats, nursery furniture that earns its space, and baby technology that genuinely helps.</p></section>
